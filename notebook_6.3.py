...
for c_step in range (n_steps):
  # Do gradient descent step
  phi_all[:,c_step+1:c_step+2] = gradient_descent_step_fixed_learning_rate(phi_all[:,c_step:c_step+1],data, alpha =0.2) # bylo 0.2
  # Measure loss and draw model every 4th step
  if c_step % 4 == 0:
    loss =  compute_loss(data[0,:], data[1,:], model, phi_all[:,c_step+1:c_step+2])
    draw_model(data,model,phi_all[:,c_step+1], "Iteration %d, loss = %f"%(c_step+1,loss))
...
# What happens if you set it too small? Zbyt mały krok powoduje bardzo powolną zbieżność, zbyt duży krok powoduje "przeskakiwanie" minimum i brak zbieżności.
...
def stochastic_gradient_descent_step(phi, data, alpha, batch_size):
  ...
  batch_x = data[0, batch_index]
  batch_y = data[1, batch_index]
  batch = np.array([batch_x, batch_y])

  phi = gradient_descent_step_fixed_learning_rate(phi, batch, alpha)
...
for c_step in range (n_steps):
  ...
  if (c_step % M == 0) and (c_step != 0) :
    alpha = alpha - beta
  phi_all[:,c_step+1:c_step+2] = stochastic_gradient_descent_step(phi_all[:,c_step:c_step+1],data, alpha, batch_size=20)
